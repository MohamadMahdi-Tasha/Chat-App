    ...
    return (
        <div className={'p-fixed top-0 transition left-0 w-100 h-100 d-flex jc-center ai-center bg-lighter-dark show-on-click'} id={props.id}>
            <div className={'bg-white rounded shadow w-35 ov-hidden'}>
                <div className={'w-100 p-1 d-flex jc-end ai-center'}><button onClick={() => {closeModal(props.id)}} className={'w-3rem h-3rem bg-none hover-bg-light-black circle border-none transition'}><i className={'bi bi-x-lg'}></i></button></div>
                <div className={'p-1'}>{props.children}</div>
                <button onClick={() => {closeModal(props.id)}} className={'w-100 p-1 bg-blue c-white border-none active-bg-blue transition'}>Change</button>
            </div>
        </div>
    );
}
